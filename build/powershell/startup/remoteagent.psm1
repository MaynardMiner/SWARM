...
function Global:start-update {
    ...
    if ($StartUpdate -eq $true) {
        ...
        $PreviousVersions = Get-ChildItem (Split-Path $(vars).Dir) | 
        Where Attributes -eq "Directory" |
        Where BaseName -like "*SWARM.*" |
        Where BaseName -ne "SWARM.$($Global:Version).linux" |
        Where Name -ne "SWARM.$($Global:Version).windows" |
        Where Name -ne "SWARM.$($Global:Version)"

        $PreviousVersions = $PreviousVersions.Name
                            ...
                            $GetOld_Json | foreach {
                                ...
                                if ($ChangeFile -notin $Exclude) {
                                    ...
                                    if ($ChangeFile -eq "t-rex.json") {
                                        $Data | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | foreach {
                                            if ($_ -ne "name") {
                                                $Data.$_.commands | Add-Member "progpow" "" -ErrorAction SilentlyContinue
                                                $Data.$_.difficulty | Add-Member "progpow" "" -ErrorAction SilentlyContinue 
                                                $Data.$_.naming | Add-Member "progpow" "progpow" -ErrorAction SilentlyContinue
                                                $Data.$_.fee | Add-Member "progpow" 1 -ErrorAction SilentlyContinue
