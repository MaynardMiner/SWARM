...
Function Get-BusFunctionID { 
    ...
    for($i=0; $i -lt $Devices.length; $i++) { 
        ...
        if(!($Devices[$i].PNPDeviceID -match "PCI\\VEN_10DE*" -and $Devices[$i].Name -ne "High Definition Audio Controller" -or $Devices[$i].PNPDeviceID -match "PCI\\VEN_1002*" -and $Devices[$i].Name -ne "High Definition Audio Controller")) {
            continue
        }
