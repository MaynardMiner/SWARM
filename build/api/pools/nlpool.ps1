function Get-NlpoolData {
    ...
    $Type | % {
        ...
        if ($Wallets -notcontains $User_Wallet) {try {$HTML = Invoke-WebRequest -Uri "http://www.nlpool.nl/site/wallet_miners_results?address=$User_Wallet" -TimeoutSec 5 -ErrorAction Stop}catch {Write-Warning "Failed to get Shares from $Pool"}}
        $Wallets += $User_Wallet
        $string = $HTML.Content
