function Get-BlockmastersData {
    ...
    $Type | % {
        ...
        if ($Wallets -notcontains $User_Wallet) {try {$HTML = Invoke-WebRequest -Uri "http://blockmasters.co/site/wallet_miners_results?address=$User_Wallet" -TimeoutSec 5 -ErrorAction Stop}catch {Write-Warning "Failed to get Shares from $Pool"}}
        $Wallets += $User_Wallet
        $string = $HTML.Content
        ...
        $string | % {
            $Cur = $_
            $CoinName = $Cur -split "</strong></td><td>" | Select -First 1;
            $Algo = $CoinName
