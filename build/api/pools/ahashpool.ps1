function Global:Get-AhashpoolData {
    ...
    $(arg).Type | ForEach-Object {
        ...
        if ($Wallets -notcontains $User_Wallet) { try { $HTML = Invoke-WebRequest -Uri "https://www.ahashpool.com/wallet_wallet_miners_results.php?wallet=$User_Wallet&showdetails=1" -TimeoutSec 10 -ErrorAction Stop }catch { log "Failed to get Shares from $Pool" } }
        $Wallets += $User_Wallet
        $string = $HTML.Content
