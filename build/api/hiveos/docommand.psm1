...
function Start-Webcommand {
    ...
    Switch ($Command.result.command) { 
        "timeout" {
            ...
            $SendResponse = Invoke-RestMethod "$($global:config.$Param.HiveMirror)/worker/api" -TimeoutSec 10 -Method POST -Body $DoResponse -ContentType 'application/json'
            $trigger = "exec"
        }
        ...
        "exec" {
            ...
            Switch ($firstword) {
                "nvidia-smi" {
                    ...
                    $DoResponse = $DoResponse | ConvertTo-JSon -Depth 1
                    $SendResponse = Invoke-RestMethod "$($global:config.$Param.HiveMirror)/worker/api" -TimeoutSec 15 -Method POST -Body $DoResponse -ContentType 'application/json'
                    Write-Host $method $messagetype $data
                    $trigger = "exec"
                ...
                "ps" {
                    ...
                    Start-Process "pwsh" -ArgumentList "-executionpolicy bypass -command `"$pscommand | Tee-Object `"$($global:Config.var.dir)\build\txt\getcommand.txt`"`"" -Verb RunAs -Wait
                    $getpayload = Get-Content ".\build\txt\getcommand.txt"
                    $line = @()
                    $getpayload | foreach { $line += "$_`n" }
                    $payload = $line
                ...
                "get" {
                    ...
                    if ($data -eq "get update") {
                        ...
                        $NewLocation = Join-Path (Split-Path $global:Config.var.dir) "SWARM.$VersionNumber"
                        $FileName = join-path "$($global:Config.var.dir)\x64" "SWARM.$VersionNumber.zip"
                        $DLFileName = Join-Path "$($global:Config.var.dir)" "x64\SWARM.$VersionNumber.zip"
                        if ($URI) {
                        ...
                        Start-Sleep -S 5
                        if ($Failed -eq $false) {
                            Start-Process "$($global:Config.var.dir)\build\apps\7z.exe" "x `"$($DLFileName)`" -o`"$($Location)`" -y" -Wait -WindowStyle Minimized
                            Start-Sleep -S 3
                            $line += "Config Command Initiated- Restarting SWARM`n"
                            Write-Host "Config Command Initiated- Restarting SWARM"
                            $MinerFile = "$($global:Config.var.dir)\build\pid\miner_pid.txt"
                            if (Test-Path $MinerFile) { $MinerId = Get-Process -Id (Get-Content $MinerFile) -ErrorAction SilentlyContinue }
                            if ($MinerId) {
            ...
            if ($command.result.config) {
                ...
                if ($OldHiveKeys) {
                    if ($NewHiveKeys.HivePassword -ne $OldHiveKeys.HivePassword) {
                        ...
                        $SendResponse = Invoke-RestMethod "$($global:config.$Param.HiveMirror)/worker/api" -TimeoutSec 15 -Method POST -Body $DoResponse -ContentType 'application/json'
                        $SendResponse
                        $DoResponse = @{method = "password_change_received"; params = @{rig_id = $global:config.$Param.HiveID; passwd = $global:config.$Param.HivePassWord }; jsonrpc = "2.0"; id = "0" }
                        $DoResponse = $DoResponse | ConvertTo-JSon -Depth 1
                        $Send2Response = Invoke-RestMethod "$($global:config.$Param.HiveMirror)/worker/api" -TimeoutSec 15 -Method POST -Body $DoResponse -ContentType 'application/json'
                    }
                }
