...
function Start-Webcommand {
    ...
    Switch ($Command.result.command) { 
        "timeout" {
            ...
            $SendResponse = Invoke-RestMethod "$($global:config.hive_params.HivePassWord)/worker/api" -TimeoutSec 10 -Method POST -Body $DoResponse -ContentType 'application/json'
            $trigger = "exec"
        }
        ...
        "exec" {
            ...
            Switch ($firstword) {
                "nvidia-smi" {
                    ...
                    $DoResponse = $DoResponse | ConvertTo-JSon -Depth 1
                    $SendResponse = Invoke-RestMethod "$($global:config.hive_params.HivePassWord)/worker/api" -TimeoutSec 15 -Method POST -Body $DoResponse -ContentType 'application/json'
                    Write-Host $method $messagetype $data
                    $trigger = "exec"
            ...
            if ($command.result.config) {
                ...
                if ($OldHiveKeys) {
                    if ($NewHiveKeys.HivePassword -ne $OldHiveKeys.HivePassword) {
                        ...
                        $DoResponse = $DoResponse | ConvertTo-JSon -Depth 1
                        $Send2Response = Invoke-RestMethod "$($global:config.hive_params.HivePassWord)/worker/api" -TimeoutSec 15 -Method POST -Body $DoResponse -ContentType 'application/json'
                    }
                }
