...
function Get-Statsxmrigopt {
    ...
    if ($Request) {
        ...
        try { for ($i = 0; $i -lt $Devices.Count; $i++) { $GPU = $Devices[$i]; $global:CPUHashrates.$GPU = ($Hash[$GPU] | Select-Object -First 1) / 1000 } }catch { Write-Host "Failed To parse threads" -ForegroundColor Red };
