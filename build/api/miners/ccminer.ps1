function Get-StatsCcminer {
    ...
    if ($GetThreads) {
        ...
        $Hash = $Null; $Hash = $Data -split ";" | Select-String "KHS" | ForEach-Object { $_ -replace ("KHS=", "") }
        ...
        try { $global:BMinerACC += $Request -split ";" | Select-String "ACC=" | ForEach-Object { $_ -replace ("ACC=", "") } }catch { }
