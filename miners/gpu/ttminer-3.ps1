if($nvidia.ttminer.path3){$Path = "$($nvidia.ttminer.path3)"}
else{$Path = "None"}
...
if($CoinAlgo -eq $null)
{
  ...
  [PSCustomObject]@{
    ...
    Symbol = "$($_.Algorithm)"
      ...
      Devices = $Devices
      DeviceCall = "ttminer"
      Arguments = "-a $($Config.$ConfigType.naming.$($_.Algorithm)) --nvidia -o stratum+tcp://$($_.Host):$($_.Port) --api-bind 0.0.0.0:2069 -u $($_.User3) -p $($_.Pass3) $($Config.$ConfigType.commands.$($_.Algorithm))"
