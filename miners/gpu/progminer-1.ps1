...
if($NVIDIA.progminer.path1){$Path = "$($NVIDIA.progminer.path1)"}
else{$Path = "None"}
...
if($CoinAlgo -eq $null)
{
  ...
  Arguments = "-U -P stratum+tcp://$($_.User1)@$($_.Host):$($_.Port) --api-port -2444 $($Config.$ConfigType.commands.$($_.Algorithm))"
