...
if ($nvidia.energiminer.path3) {$Path = "$($nvidia.energiminer.path3)"}
else {$Path = "None"}
...
##Parse -GPUDevices
if ($NVIDIADevices3 -ne "none") {$GPUDevices3 = $NVIDIADevices3}
if ($GPUDevices3 -ne '') {
        ...
        $AlgoPools | Where Symbol -eq $MinerAlgo | foreach {
            if ($Algorithm -eq "$($_.Algorithm)" -and $Bad_Miners.$($_.Algorithm) -notcontains $Name) {
                ...
                [PSCustomObject]@{
                    ...
                    Arguments  = "-U stratum://$($_.User3).$($_.Pass3)@$($_.Algorithm).mine.zergpool.com:$($_.Port)"
