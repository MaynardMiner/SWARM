...
if($nvidia.ewbf.path3){$Path = "$($nvidia.ewbf.path3)"}
else{$Path = "None"}
...
if($CoinAlgo -eq $null)
{
  ...
  [PSCustomObject]@{
      ...
      Devices = $Devices
      DeviceCall = "ewbf"
      Arguments = "--api 0.0.0.0:42002 --server $($_.Host) --port $($_.Port) --user $($_.User3) --pass $($AlgoPools.$_.Pass3)$($Diff) $($Config.$ConfigType.commands.$($_.Algorithm))"
