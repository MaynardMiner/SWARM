if ($nvidia.phoenix.path1) { $Path = "$($nvidia.phoenix.path1)" }
else { $Path = "None" }
...
$Config.$ConfigType.commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object {
    ...
    $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
        if ($Algorithm -eq "$($_.Algorithm)" -and $Bad_Miners.$($_.Algorithm) -notcontains $Name) {
            ...
            else { $MinerWorker = "-pass $($_.Pass1)$($Diff) " }
            [PSCustomObject]@{
                ...
                Devices    = $Devices
                DeviceCall = "claymore"
                Arguments  = "-platform 2 -mport 5333 -mode 1 -allcoins 1 -allpools 1 -pool $($_.Protocol)://$($_.Host):$($_.Port) -wal $($_.User1) $MinerWorker-wd 0 -logfile `'$(Split-Path $Log -Leaf)`' -logdir `'$(Split-Path $Log)`' -gser 2 -dbg -1 -eres 1 $($Config.$ConfigType.commands.$($_.Algorithm))"
