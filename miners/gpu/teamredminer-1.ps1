...
if ($amd.teamredminer.path1) { $Path = "$($amd.teamredminer.path1)" }
else { $Path = "None" }
    ...
    $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
        if ($Algorithm -eq "$($_.Algorithm)" -and $Bad_Miners.$($_.Algorithm) -notcontains $Name) {
            ...
            [PSCustomObject]@{
                ...
                Devices    = $Devices
                DeviceCall = "tdxminer"
                Arguments  = "--platform $AMDPlatform -a $($Config.$ConfigType.naming.$($_.Algorithm)) --api_listen=0.0.0.0:4008 -o stratum+tcp://$($_.Host):$($_.Port) -u $($_.User1) --log_file `'$Log`' -p $($_.Pass1)$($DIff) $($Config.$ConfigType.commands.$($_.Algorithm))"
