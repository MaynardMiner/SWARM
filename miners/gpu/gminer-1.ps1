...
if ($NVIDIADevices1 -ne "none") {
    ...
    $GPUEDevices1 | % {$ArgDevices += "$($GCount.NVIDIA.$_) " }
        ...
        $Pools | Where Algorithm -eq $MinerAlgo | foreach {
            if ($Algorithm -eq "$($_.Algorithm)" -and $Bad_Miners.$($_.Algorithm) -notcontains $Name) {
                ...
                [PSCustomObject]@{
                    ...
                    DeviceCall = "gminer"
                    Arguments  = "--api 42000 --server $($_.Host) --port $($_.Port) --user $($_.User1) --logfile `'$Log`' --pass $($_.Pass1)$Diff $($Config.$ConfigType.commands.$($_.Algorithm))"
