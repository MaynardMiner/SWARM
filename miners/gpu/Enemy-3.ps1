...
if($nvidia.enemy.path3){$Path = "$($nvidia.enemy.path3)"}
else{$Path = "None"}
...
if($CoinAlgo -eq $null)
{
  ...
  Devices = $Devices
  DeviceCall = "ccminer"
  Arguments = "-a $($Config.$ConfigType.naming.$($_.Algorithm)) -o stratum+tcp://$($_.Host):$($_.Port) -b 0.0.0.0:4071 -u $($_.User3) -p $($_.Pass3)$($Diff) $($Config.$ConfigType.commands.$($_.Algorithm))"
