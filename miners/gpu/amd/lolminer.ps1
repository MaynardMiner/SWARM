$AMDTypes | ForEach-Object {
    ...
    if ($amd.lolminer.$ConfigType) { $Path = "$($amd.lolminer.$ConfigType)" }
    else { $Path = "None" }
    ...
    $GetConfig = "$($global:Dir)\config\miners\lolminer.json"
            ...
            if ($Check.RAW -ne "Bad") {
                $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                    ...
                    [PSCustomObject]@{
                        ...
                        Devices    = $Devices
                        DeviceCall = "lolminer"
                        Arguments  = "--pool $($_.Host) --port $($_.Port) --user $($_.$User) --pass $($_.$Pass)$($Diff) --apiport $Port --logs 0 --devices AMD $($MinerConfig.$ConfigType.commands.$($_.Algorithm))"
