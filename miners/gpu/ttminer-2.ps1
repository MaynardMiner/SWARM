        ...
        $AlgoPools | Where Symbol -eq $MinerAlgo | foreach {
            if ($Algorithm -eq "$($_.Algorithm)") {
                [PSCustomObject]@{
                    ...
                    Devices    = $Devices
                    DeviceCall = "claymore"
                    Arguments  = "-a $($Config.$ConfigType.naming.$($_.Algorithm)) --nvidia -o $($_.Protocol)://$($_.Host):$($_.Port) -b localhost:2069 -u $($_.User2) -p $($_.Pass2) $($Config.$ConfigType.commands.$($_.Algorithm))"
