$(vars).NVIDIATypes | ForEach-Object {
    ...
    if ($(vars).nvidia.ethminer.$ConfigType) { $Path = "$($(vars).nvidia.ethminer.$ConfigType)" }
    else { $Path = "None" }
    if ($(vars).nvidia.ethminer.uri) { $Uri = "$($(vars).nvidia.ethminer.uri)" }
    else { $Uri = "None" }
    if ($(vars).nvidia.ethminer.minername) { $MinerName = "$($(vars).nvidia.ethminer.minername)" }
    else { $MinerName = "None" }

    $User = "User$Num"; $Pass = "Pass$Num"; $Name = "ethminer-$Num"; $Port = "5900$Num"
    ...
    $MinerConfig = $Global:config.miners.ethminer
            ...
            if ($Check.RAW -ne "Bad") {
                $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                    ...
                    [PSCustomObject]@{
                        ...
                        Arguments  = "-P stratum2+tcp://$($_.$User).$($_.$Pass):x@$($_.Pool_Host):$($_.Port) --cuda --api-bind 127.0.0.1:$Port --noeval --dag-load-mode 1 $($MinerConfig.$ConfigType.commands.$($_.Algorithm))"
                        HashRates  = $Stat.Hour
