...
$(vars).NVIDIATypes | ForEach-Object {
    ...
    if ($(vars).nvidia.'cc-lyra2z330'.$ConfigType) { $Path = "$($(vars).nvidia.'cc-lyra2z330'.$ConfigType)" }
    else { $Path = "None" }
    if ($(vars).nvidia.'cc-lyra2z330'.uri) { $Uri = "$($(vars).nvidia.'cc-lyra2z330'.uri)" }
    else { $Uri = "None" }
    if ($(vars).nvidia.'cc-lyra2z330'.minername) { $MinerName = "$($(vars).nvidia.'cc-lyra2z330'.minername)" }
    else { $MinerName = "None" }

    $User = "User$Num"; $Pass = "Pass$Num"; $Name = "cc-lyra2z330-$Num"; $Port = "5600$Num";
    ...
    $MinerConfig = $Global:config.miners.'cc-lyra2z330'
        ...
        if (
            ...
            $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                ...
                [PSCustomObject]@{
                    ...
                    Version    = "$($(vars).nvidia.'cc-lyra2z330'.version)"
                    DeviceCall = "ccminer"
