...
if($CoinAlgo -eq $null)
{
  ...
  if($Config.$ConfigType.difficulty.$_){$Diff=",d=$($Difficulty.$_)"}
   ...
   ocpower = if($Config.$ConfigType.oc.power.$_){$Config.$ConfigType.oc.power.$_}else{"$OC_$($ConfigType)".Power}
   occore = if($Config.$ConfigType.oc.core.$_){$Config.$ConfigType.oc.power.$_}else{"$OC_$($ConfigType)".core}
   ocmem = if($Config.$ConfigType.oc.memory.$_){$Config.$ConfigType.oc.power.$_}else{"$OC_$($ConfigType)".memory}
