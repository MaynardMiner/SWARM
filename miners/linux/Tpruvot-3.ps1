...
if($Algorithm -eq "$($AlgoPools.$_.Algorithm)")
 ...
 if($Config.$ConfigType.difficulty.$_){$Diff=",d=$($Difficulty.$_)"}
  ...
  Where {$($Config.$ConfigType.commands.$($CoinPools.$_.Algorithm)) -NE $null} |
   ...
   Devices = $Devices
   DeviceCall = "ccminer"
   Arguments = "-a $(Get-Nvidia($CoinPools.$_.Algorithm)) -o stratum+tcp://$($CoinPools.$_.Host):$($CoinPools.$_.Port) -b 0.0.0.0:4070 -u $($CoinPools.$_.User3) -p $($CoinPools.$_.Pass3)$($Diff) $($Config.$ConfigType.commands.$($Coinpools.$_.Algorithm))"
