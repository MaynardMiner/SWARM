...
if ($Name -in $(arg).PoolName) {
    ...
    $Pool_Sorted | ForEach-Object {
        ...
        if (-not $(vars).Pool_Hashrates.$($_.Name)) {
            $(vars).Pool_Hashrates.Add("$($_.Name)", @{ }) 
        }
        if (-not $(vars).Pool_Hashrates.$($_.Name).$Name) {
            $(vars).Pool_Hashrates.$($_.Name).Add("$Name", @{HashRate = "$($Stat.HashRate)"; Percent = "" })
        }
