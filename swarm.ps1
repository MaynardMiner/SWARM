...
param(
    ...
    [Parameter(Mandatory = $false)]
    [array]$ASIC_ALGO, ##Array of algos used for ASIC
    [Parameter(Mandatory = $false)]
    [String]$Stat_All = "No", ## If set to yes- Will stat all coins, not just most profitible (High Resource Use)
    [Parameter(Mandatory = $false)]
...
Write-log "OS = $Platform" -ForegroundColor Green
...
While ($true) {
    ...
    do {
        ...
        ##Get Price Data
        try {
            write-Log "SWARM Is Building The Database. Auto-Coin Switching: $Auto_Coin" -foreground "yellow"
                            ...
                            if ($_.WasBenchmarked -eq $False) {
                                ...
                                if ($null -eq $Miner_HashRates -or $Miner_HashRates -eq 0) {
                                    ...
                                    if ($ScreenCheck -eq "0.00 PH" -or $null -eq $StatCheck) {
                                        ...
                                        if ($WattOmeter -eq "Yes") { write-Log "Watt-O-Meter scored $($_.Name) $($_.Symbol) at $($GPUPower) Watts" -ForegroundColor magenta }
...
" -ForegroundColor DarkRed
                                ...
                                $HiveWarning = @{result = @{command = "timeout" } }
                                if ($HiveOS -eq "Yes") { try { $SendToHive = Start-webcommand -command $HiveWarning -swarm_message $HiveMessage -HiveID $HiveId -HivePassword $HivePassword -HiveMirror $HiveMirror }catch { Write-Log "WARNING: Failed To Notify HiveOS" -ForeGroundColor Yellow } }
                                Start-Sleep -S 1
                            }
